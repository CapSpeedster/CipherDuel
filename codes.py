...
def aristocratk3(text, key, shift, LETTERS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ '):
    text=text_clean(text, LETTERS)
    plainkey=form_cipher(text_clean(key), shift)
    cipherkey=form_cipher(text_clean(key), 0)
    plaintext=''
    for i in text:
        if i == ' ':
            plaintext+=' '
            continue
        ciphertext_num = cipherkey.find(i)
        plaintext+=plainkey[ciphertext_num]
    return plaintext
